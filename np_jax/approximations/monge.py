...
def run_monge(
    ...
    try:
        ...
        if fisher_precision:
            ...
            if save_figures:
                fig_name = f"f_{name}_monge.png"
            if save_samples:
                samples_name = f"f_{name}_monge_samples.npy"
                num_evals_name = f"f_{name}_monge_num_evals.npy"
        else:
            show("Hessian precision")
            if save_figures:
                fig_name = f"h_{name}_monge.png"
            if save_samples:
                samples_name = f"h_{name}_monge_samples.npy"
                num_evals_name = f"h_{name}_monge_num_evals.npy"
        ...
        if use_diffrax:
            ...
            def func(num_sample):
                velocity = get_np_velocity(dim, precision, temp_samples[num_sample])[
                    "y"
                ][dim:, 0]
                return velocity

            # https://stackoverflow.com/a/71977764
            velocities = Parallel(n_jobs=-1, timeout=99999)(
            ...
            samples_results, samples, num_evals = mapped_func(velocities)
        ...
        if ground_truth_samples is not None:
            ...
            if fisher_precision:
                results["Monge_fisher_precision"] = {
                    "W": distance,
                    "num_evals": [mean_num, std_num],
                    "max_steps_exceeded": max_steps_exceeded,
                }
            else:
                results["Monge_hessian_precision"] = {
                    "W": distance,
                    "num_evals": [mean_num, std_num],
                    "max_steps_exceeded": max_steps_exceeded,
                }
